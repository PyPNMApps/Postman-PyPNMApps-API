...
function constructVisualizerPayload() {
  ...
  const rowsNorm = results.map((item, idx) => {
    ...
    // docsIf3SignalQualityExtRxMER is typically tenths of dB (common), your sample looks like 433 => 43.3 dB
    const rxMerRaw = entry.docsIf3SignalQualityExtRxMER;
    const rxmerDb = (typeof rxMerRaw === 'number') ? (rxMerRaw / 10.0) : null;
